! function(e, t, i, n) {
  ...
  i.extend(s, {
    ...
    _setDimension: function() {
      ...
      if (T = g(a), E = g(r), d = a / r, L = g(f(L) ? g(L, "w") - n : L), P = g(f(P) ? g(P, "w") - n : P), H = g(f(H) ? g(H, "h") - o : H), R = g(f(R) ? g(R, "h") - o : R), l = P, c = R, S.fitToView && (P = Math.min(w.w - n, P), R = Math.min(w.h - o, R)), u = w.w - I, y = w.h - D, S.aspectRatio ? (T > P && (T = P, E = g(T / d)), E > R && (E = R, T = g(E * d)), T < L && (T = L, E = g(T / d)), E < H && (E = H, T = g(E * d))) : (T = Math.max(L, Math.min(T, P)), S.autoHeight && "iframe" !== S.type && (_.width(T), E = _.height()), E = Math.max(H, Math.min(E, R))), S.fitToView)
        if (_.width(T).height(E), O.width(T + e), p = O.width(), h = O.height(), S.aspectRatio)
          for (;
            (p > u || h > y) && T > L && E > H && !(b++ > 19);) E = Math.max(H, Math.min(R, E - 10)), T = g(E * d), T < L && (T = L, E = g(T / d)), T > P && (T = P, E = g(T / d)), _.width(T).height(E), O.width(T + e), p = O.width(), h = O.height();
    ...
    create: function(e) {
      var t;
      e = i.extend({}, this.defaults, e), this.overlay && this.close(), t = s.coming ? s.coming.parent : e.parent, this.overlay = i('<div class="fancybox-overlay"></div>').appendTo(t && t.lenth ? t : "body"), this.fixed = !1, e.fixed && s.defaults.fixed && (this.overlay.addClass("fancybox-overlay-fixed"), this.fixed = !0)
    },
    open: function(e) {
      var t = this;
