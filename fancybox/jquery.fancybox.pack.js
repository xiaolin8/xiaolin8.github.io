  ...
  i.extend(l, {
    ...
    _setDimension: function() {
      var e, t, n, o, a, r, s, c, d, p = l.getViewport(),
        ...
        E = u(T[1] + T[3]),
        L = u(T[0] + T[2]);
      if (m.add(x).add(v).width("auto").height("auto").removeClass("fancybox-tmp"), T = u(x.outerWidth(!0) - x.width()), e = u(x.outerHeight(!0) - x.height()), t = E + T, n = L + e, o = f(y) ? (p.w - t) * u(y) / 100 : y, a = f(b) ? (p.h - n) * u(b) / 100 : b, "iframe" === w.type) {
      ...
      if (y = u(o), b = u(a), c = o / a, k = u(f(k) ? u(k, "w") - t : k), O = u(f(O) ? u(O, "w") - t : O), C = u(f(C) ? u(C, "h") - n : C), W = u(f(W) ? u(W, "h") - n : W), r = O, s = W, w.fitToView && (O = Math.min(p.w - t, O), W = Math.min(p.h - n, W)), t = p.w - E, L = p.h - L, w.aspectRatio ? (y > O && (y = O, b = u(y / c)), b > W && (b = W, y = u(b * c)), y < k && (y = k, b = u(y / c)), b < C && (b = C, y = u(b * c))) : (y = Math.max(k, Math.min(y, O)), w.autoHeight && "iframe" !== w.type && (v.width(y), b = v.height()), b = Math.max(C, Math.min(b, W))), w.fitToView)
        if (v.width(y).height(b), m.width(y + T), p = m.width(), E = m.height(), w.aspectRatio)
          for (;
            (p > t || E > L) && y > k && b > C && !(19 < h++);) b = Math.max(C, Math.min(W, b - 10)), y = u(b * c), y < k && (y = k, b = u(y / c)), y > O && (y = O, b = u(y / c)), v.width(y).height(b), m.width(y + T), p = m.width(), E = m.height();
        else y = Math.max(k, Math.min(y, y - (p - t))), b = Math.max(C, Math.min(b, b - (E - L)));
      S && "auto" === _ && b < a && y + T + S < t && (y += S), v.width(y).height(b), m.width(y + T), p = m.width(), E = m.height(), m = (p > t || E > L) && y > k && b > C, y = w.aspectRatio ? y < r && b < s && y < o && b < a : (y < r || b < s) && (y < o || b < a), i.extend(w, {
        dim: {
          width: g(p),
          height: g(E)
        },
